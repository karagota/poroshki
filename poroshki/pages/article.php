...
<div class="col-12 article" style="padding:0 15px;" id="article-<?php echo $article['id'];?>">
  <h3>
  <?php
	if ($user->is_logged() && ($article['author_id']==$_SESSION['user_id'])) {
		if ($article['status']==0 && $article['id']>1) {
			echo '&nbsp;&nbsp;&nbsp;<a href="/create/'.$article['id'].'" style="color:green;" id="id-'.$article['id'].'" title="'.$labels['edit_draft'].'"><span class="glyphicon glyphicon-pencil"></span></a></li>';
		}
		/*else echo '&nbsp;&nbsp;&nbsp;<a href="#" style="color:green;" id="id-'.$article['id'].'" title="Убрать статью в черновики" class="change-status"><span class="glyphicon glyphicon-eye-close"></span><span style="font-size:10px;">В черновики</span></a></li>';*/
  ...
  <?php if ($article['status']==1 ) { 
		echo infobar($article,$user,$annulate,$vote=1,$info=0);	
	}
   ?>
...
<div style="margin-bottom:10px;">
 ...
 <?php 
	if ($article['status']==1) { 
		echo infobar($article,$user,$annulate,$vote=0,$info=1);	
	}
?>
 <?php 
 if ($user->is_logged() && ($article['author_id']==$_SESSION['user_id']) && $article['id']>1) {
					echo '<a href="/'.$article['id'].'?ac=1"  id="id-'.$fav['id'].'" title="'.$labels['edit_author_comment'].'"><span class="glyphicon glyphicon-pencil"></span></a>';
					
				}
?>
 <?php if (empty($_GET['ac']) &&!empty($article['comment'])) { ?>
	 <?php /*<h3><?php echo $labels['author_comment_title'];?>:</h3>*/ ?>

	<span class="tooltip-demo" title="<?php echo $labels['author_comment_title'];?>"> <?php echo $article['comment'];?></span>
 <?php } ?>
<?php /*
 <hr />
  <div id="author_comment" style="margin-top:0px;margin-bottom:0px;"></div>
*/ ?>
<?php if ($user->is_logged() && ($article['author_id']==$_SESSION['user_id']) && $_GET['ac']==1 && $article['id']>1 ){?>
 <h4 ><?php echo $labels['edit_author_comment'];?>:</h4>
<form method="post" class="postform">
 
  <textarea name="text" class="form-control" ><?php echo $article['comment'];?></textarea>
  <input type="hidden"  name="author_comment_article_id" value="<?php echo $article['id'];?>" />
  <br />
<button class="btn primary" type="submit" name="submit"><?php echo $labels['submit'];?></button>
</form>
<?php }?>
...
<?php if ($article['status']==1 || $article['id']==1) { ?>		
      ...
      <div class="btn-group">
	   ...
	   <div class="btn-group">
        ...
        <a class="btn" data-edit="redo" title="Вернуть (Ctrl/Cmd+Y)"><i class="icon-repeat"></i></a>
 	    ...
 	    <?php /*<a class="btn" title="Вставить картинку (или просто drag & drop)" id="pictureBtn"><i class="icon-picture"></i></a>
