...
function display_report(){
	...
	{
		$note= $labels['rating'].' на дату '.$_POST['from'].' берется из значения, установленного при создании автора.';
	...
	$sql = "select grade, vote.since as since, voter_id, vote.subject_id as article,  rating from vote,rating where rating.subject_type=0 and rating.subject_id=vote.voter_id and  vote.subject_type=1 and vote.subject_id in (select id from articles where author_id=$who) and since between '$from' and '$to'";
	//echo $sql.'<br>';
	$res = mysql_query($sql);
	while ($row = mysql_fetch_assoc($res)) {
		$votes[] = $row;
	...
	echo '<tr><td>Дата</td><td>'.$labels['rating'].' голосовавшего</td><td>За что голосовал</td><td>Голос</td></tr>';
	foreach ($votes as $vote) {
		echo '<tr><td>'.$vote['since'].'</td><td>'.rating($vote['rating']).'</td><td>'.$vote['article'].'</td><td>'.$vote['grade'].'</td></tr>';
	}
	echo '</table><br>';
